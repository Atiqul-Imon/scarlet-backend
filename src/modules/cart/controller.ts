...
export async function addItem(req: any, res: any) {
  ...
  ok(res, await presenter.addItem(req.userId, { productId, quantity: Math.floor(quantity), selectedSize }));
}
...
export async function addGuestItem(req: any, res: any) {
  ...
  if (!productId || typeof quantity !== 'number' || quantity < 1)
    return fail(res, { message: 'productId and quantity>=1 required' }, 400);
  ok(res, await presenter.addGuestItem(sessionId, { productId, quantity: Math.floor(quantity), selectedSize }));
}
