  ...
  try {
    ...
    const assignCategories = (product, section) => {
      ...
      if (section === 'new-arrivals') {
        if (product.tags.some(t => ['serum', 'moisturizer', 'skincare-essential'].includes(t))) {
          categoryIds.push(categoryMap['skincare']);
        }
        if (product.tags.some(t => ['lipstick', 'lip-oil', 'lip-balm', 'makeup'].includes(t))) {
          categoryIds.push(categoryMap['makeup']);
        }
      }
    ...
    // Insert products
    console.log('🛍️ Inserting 9 additional products...');
    ...
    console.log('\n🎉 Success! Homepage now has 15 products per section (45 total)');
